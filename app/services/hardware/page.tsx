...
import { Monitor, Cpu, Network, Wrench, Camera, Briefcase, Laptop, ArrowUpRight, ShieldCheck, Zap } from "lucide-react"
...
export default function HardwareServices() {
    ...
    const items = [
        { key: "servers", icon: Cpu, image: "/services/hardware/Server-&-Workstation-Preparation.jpg" },
        { key: "networks", icon: Network, image: "/services/hardware/Network-&-Cabling-Equipment.jpg" },
        { key: "maintenance", icon: Wrench, image: "/services/hardware/Maintenance-&-Tech-Support-Contracts.jpg" },
        { key: "cctv", icon: Camera, image: "/services/hardware/CCTV-&-Security-Systems.jpg" },
        { key: "field_support", icon: Briefcase, image: "/services/hardware/Field-Technical-Support.jpg" },
        { key: "workstations", icon: Laptop, image: "/services/hardware/Workstations-&-Laptops.jpg" },
    ]
    ...
    return (
        <main dir={lang === "ar" ? "rtl" : "ltr"} className="bg-bg-primary min-h-screen text-text-primary overflow-hidden">
            ...
            <PageSection className="pb-32!">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
                    {items.map((item, i) => (
                        ...
                        >
                            <Link href={`/services/hardware/${item.key}`} className="block h-full group">
                                <Card className="h-full p-0 overflow-hidden border-border-color hover:border-accent/40 bg-bg-secondary/20 transition-all duration-700 flex flex-col shadow-2xl hover:shadow-4xl relative">
                                    ...
                                    <div className="p-10 md:p-12 flex flex-col grow">
                                        ...
                                        <h3 className="text-2xl md:text-3xl font-black text-text-primary mb-6 group-hover:text-accent transition-colors tracking-tighter leading-tight uppercase">
                                            {t(`services_pages.hardware.${item.key}`)}
                                        </h3>
                                        <p className="text-text-secondary text-lg leading-relaxed grow mb-12 max-w-sm font-medium opacity-70 group-hover:opacity-100 transition-opacity">
                                            {t(`services_pages.hardware.${item.key}_desc`)}
                                        </p>

                                        <div className="relative aspect-4/3 w-full rounded-[2.5rem] overflow-hidden border border-border-color bg-bg-primary shadow-2xl group-hover:border-accent/40 transition-all duration-700">
                                            <Image
                                                src={item.image}
                                                alt={item.key}
                                                fill
            ...
            <PageSection className="bg-bg-secondary/30 border-y border-border-color py-40!" columns={2}>
                ...
                <div className="flex flex-col justify-center lg:ps-20 order-1 lg:order-2 space-y-12">
                     <motion.div
                            ...
                            {[
                                ...
                                >
                                    ...
                                    <span className="text-text-primary text-sm font-black uppercase tracking-widest group-hover/badge:text-accent transition-colors">
                                        {t(`services_pages.hardware.impact.badges.${item.key}`)}
                                    </span>
                                </motion.div>
                            ))}
                        </div>
