...
export default function CartPage() {
    ...
    return (
        <div className="min-h-screen bg-[#050505] pt-32 pb-20 px-4 md:px-6 font-sans text-white" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
            <div className="max-w-6xl mx-auto">
                ...
                {items.length === 0 ? (
                    <div className="text-center py-32">
                        <div className="w-24 h-24 rounded-3xl bg-zinc-900 border border-zinc-800 flex items-center justify-center mx-auto mb-6">
                    ...
                    <div className="grid lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-4">
                            {items.map((item) => {
                                ...
                                return (
                                    <div key={item.id} className="flex gap-4 md:gap-6 p-4 md:p-6 rounded-3xl bg-zinc-950 border border-white/5 hover:border-white/10 transition-all group">
                                        {/* Image */}
                                        <Link href={`/store/${item.id}`} className="shrink-0">
                                            <div className="relative w-24 h-24 md:w-32 md:h-32 rounded-2xl bg-zinc-900 overflow-hidden">
                                                <Image
                                        ...
                                        <div className="flex-1 min-w-0 flex flex-col justify-between">
                                            <div>
                                                <Link href={`/store/${item.id}`}>
                                                    <h3 className="font-medium text-white text-sm md:text-base truncate hover:text-[#0066FF] transition-colors">{name}</h3>
                                                </Link>
                                                <div className="flex items-center gap-2 mt-1">
                                                    <span className="text-lg md:text-xl font-bold text-white">{itemPrice} {lang === 'ar' ? 'ج.م' : 'EGP'}</span>
                                                    {item.discountPrice && (
                                                        <span className="text-xs text-zinc-500 line-through">{item.price} {lang === 'ar' ? 'ج.م' : 'EGP'}</span>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center justify-between mt-4">
                                                <div className="flex items-center gap-1 bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden">
                                                    <button
                                                        ...
                                                        className="w-9 h-9 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-zinc-800 transition-all"
                                                    >
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-9 h-9 flex items-center justify-center text-zinc-400 hover:text-white hover:bg-zinc-800 transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                                                    >
                            ...
                            <button
                                ...
                                className="text-sm text-zinc-600 hover:text-red-400 transition-colors font-medium mt-2"
                            >
                        ...
                        <div className="lg:col-span-1">
                            <div className="sticky top-24 rounded-3xl bg-zinc-950 border border-white/5 p-6 md:p-8 space-y-6">
                                ...
                                <div className="space-y-4 text-sm">
                                    <div className="flex justify-between text-zinc-400">
                                        ...
                                        <span className="text-white font-bold">{totalPrice.toFixed(2)} {lang === 'ar' ? 'ج.م' : 'EGP'}</span>
                                    </div>
                                    <div className="flex justify-between text-zinc-400">
                                ...
                                <Button
                                    ...
                                    className="w-full h-14 rounded-2xl bg-[#0066FF] hover:bg-white hover:text-black font-black text-base shadow-[0_0_40px_rgba(0,102,255,0.3)] transition-all disabled:opacity-50"
                                >
                                    {isCheckingOut ? (
