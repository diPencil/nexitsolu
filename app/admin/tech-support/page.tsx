...
export default function AdminTechSupport() {
    ...
    const statusColors: Record<string, string> = {
        ...
        CLOSED: 'bg-zinc-500/10 text-muted-foreground border-zinc-500/20',
    }
        ...
        <div className="space-y-8 pb-32 w-full">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    ...
                    <p className="text-muted-foreground">{lang === 'ar' ? 'مراجعة وتحديث طلبات الدعم الفني الواردة.' : 'Review and update incoming technical support inquiries.'}</p>
                </div>
            ...
            {isLoading ? (
                ...
                <div className="py-20 text-center bg-background rounded-4xl border border-border text-muted-foreground/60 font-bold uppercase tracking-widest text-xs">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filtered.map((req, i) => (
                        <motion.div
                            ...
                            className="p-8 rounded-[2.5rem] bg-popover border border-border hover:border-[#0066FF]/50 transition-all group overflow-hidden relative shadow-none flex flex-col"
                        >
                            ...
                            <div className="relative z-30 flex-1 flex flex-col">
                                <div className="flex justify-between items-start mb-6">
                                    ...
                                    <div className="text-end">
                                        <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest mb-1">{lang === 'ar' ? 'نوع الدعم' : 'Support Type'}</p>
                                ...
                                <div className="flex items-center gap-4 mb-4 relative z-10 transition-transform group-hover:translate-x-1">
                                    <div className="w-14 h-14 rounded-2xl bg-linear-to-br from-[#0066FF]/20 to-purple-600/20 flex items-center justify-center font-black text-xl text-foreground border border-border shrink-0">
                                        {req.name?.[0]?.toUpperCase()}
                                    </div>
                                    <div className="overflow-hidden">
                                        <h3 className="font-bold text-lg text-foreground truncate w-full">{req.name}</h3>
                                        <p className="text-xs text-muted-foreground truncate w-full">{req.email}</p>
                                    </div>
                                </div>

                                <div className="mt-auto pt-6 border-t border-border flex items-center justify-between relative z-20">
                                    ...
                                    <Button
                                        ...
                                        className="bg-[#0066FF] hover:bg-[#0066FF]/90 text-foreground border-none rounded-xl text-xs font-black h-10 w-10 p-0 shadow-lg shadow-[#0066FF]/20"
                                    >
                ...
                {activeRequest && (
                    <div className="fixed inset-0 z-100 flex items-center justify-center p-6 backdrop-blur-xl bg-black/80">
                        <motion.div
                            ...
                            className="w-full max-w-2xl bg-card border border-border shadow-none overflow-hidden flex flex-col max-h-[90vh]"
                        >
                            <div className="p-6 border-b border-border flex items-center justify-between bg-muted/50 shrink-0">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-muted-foreground uppercase tracking-widest mt-1">Status: {activeRequest.status}</p>
                                    </div>
                                </div>
                                <button onClick={() => setActiveRequest(null)} className="p-3 hover:bg-muted rounded-2xl transition-all">
                                    <X className="w-6 h-6" />
                                </button>
                            </div>
                            ...
                            <div className="flex-1 p-8 overflow-y-auto min-h-0">
                                <div className="grid grid-cols-2 gap-4 border-b border-border pb-6">
                                    <div className="p-4 rounded-3xl bg-muted border border-border">
                                        ...
                                        <p className="text-base font-bold text-foreground">{activeRequest.name}</p>
                                    </div>
                                    <div className="p-4 rounded-3xl bg-muted border border-border">
                                        <label className="text-[9px] font-black text-muted-foreground uppercase tracking-widest block mb-1">Support Type</label>
                                ...
                                <div className="py-6 border-b border-border grid md:grid-cols-2 gap-4">
                                    ...
                                    <div className="p-4 rounded-3xl bg-muted border border-border flex items-center gap-3">
                                        ...
                                        <div>
                                            <label className="text-[9px] font-black text-muted-foreground uppercase tracking-widest block mb-1">Phone/WhatsApp</label>
                                ...
                                <div className="pt-6">
                                    <h4 className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-3">{lang === 'ar' ? 'الرسالة' : 'Issue Description'}</h4>
                                    <div className="p-6 rounded-3xl bg-muted border border-border text-sm leading-relaxed text-foreground">
                                        {activeRequest.message}
                                    </div>
                                </div>
                            </div>

                            <div className="p-6 border-t border-border bg-muted/50 flex flex-wrap gap-4 shrink-0">
                                ...
                                {(activeRequest.status === 'RESOLVED' || activeRequest.status === 'IN_PROGRESS') && (
                                    <button
                                        ...
                                        className="h-12 px-6 rounded-2xl bg-muted border border-border text-foreground font-bold flex items-center justify-center gap-2 disabled:opacity-50"
                                    >
                                ...
                                <button
                                    ...
                                    className="px-6 h-12 rounded-2xl bg-muted border border-border text-red-500 hover:bg-red-500/10 font-bold transition-all"
                                >
