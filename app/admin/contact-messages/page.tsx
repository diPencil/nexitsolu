        ...
        <div className="space-y-8 pb-32">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                <div>
                    ...
                    <p className="text-zinc-500">{lang === 'ar' ? 'مراجعة رسائل الاستفسار الواردة من صفحة تواصل معنا.' : 'Review inquiry messages from the Contact Us page.'}</p>
                </div>
                <div className="flex items-center gap-4">
                    <div className="hidden sm:flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2 rounded-2xl">
                        ...
                        <span className="text-xs text-zinc-500 uppercase tracking-widest">{lang === 'ar' ? 'رسالة' : 'messages'}</span>
                    </div>
                </div>
            </div>
                        ...
                        >
                            ...
                            <div className="relative z-30">
                                ...
                                <div className="flex items-center gap-4 mb-4 relative z-10 transition-transform group-hover:translate-x-1">
                                    <div className="w-14 h-14 rounded-2xl bg-linear-to-br from-[#0066FF]/20 to-cyan-600/20 flex items-center justify-center font-black text-xl text-white border border-white/5 shrink-0">
                                        {msg.name?.[0]?.toUpperCase()}
                                    </div>
                                    <div className="overflow-hidden">
                                        <h3 className="font-bold text-lg text-white truncate w-full">{msg.name}</h3>
                                        <p className="text-xs text-zinc-500 truncate w-full">{msg.email}</p>
                                    </div>
                                </div>

                                <div className="mb-6 relative z-20">
                                    <p className="text-[10px] text-[#0066FF] font-black uppercase mb-1">{lang === 'ar' ? 'الموضوع' : 'Subject'}</p>
                                    <p className="text-sm font-bold text-white truncate">{msg.subject || (lang === 'ar' ? 'بدون موضوع' : 'No subject')}</p>
                                </div>
                ...
                {activeMessage && (
                    <div className="fixed inset-0 z-100 flex items-center justify-center p-6 backdrop-blur-xl bg-black/80">
                        ...
                        >
                            <div className="p-6 border-b border-white/5 flex items-center justify-between bg-zinc-900/50 shrink-0">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-[10px] text-zinc-500 uppercase tracking-widest mt-1">Status: {activeMessage.status}</p>
                                    </div>
                                </div>
                                <button onClick={() => setActiveMessage(null)} className="p-3 hover:bg-white/5 rounded-2xl transition-all">
                                    <X className="w-6 h-6" />
                                </button>
                            </div>
                            ...
                            <div className="flex-1 p-8 overflow-y-auto min-h-0">
                                <div className="grid grid-cols-2 gap-4 border-b border-white/5 pb-6">
                                    <div className="p-4 rounded-3xl bg-white/5 border border-white/5">
                                        ...
                                        <p className="text-base font-bold text-white">{activeMessage.name}</p>
                                    </div>
                                    <div className="p-4 rounded-3xl bg-white/5 border border-white/5">
                                ...
                                <div className="py-6 border-b border-white/5">
                                    <div className="p-4 rounded-3xl bg-white/5 border border-white/5 flex items-center gap-3">
                                        ...
                                        <div>
                                            <label className="text-[9px] font-black text-zinc-500 uppercase tracking-widest block mb-1">Phone Number</label>
                                ...
                                <div className="pt-6">
                                    <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">{lang === 'ar' ? 'الموضوع' : 'Subject'}</h4>
                                    <p className="text-lg font-bold text-white mb-6">{activeMessage.subject || 'No Subject'}</p>

                                    <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest mb-3">{lang === 'ar' ? 'الرسالة' : 'Message Content'}</h4>
                            ...
                            <div className="p-6 border-t border-white/5 bg-zinc-900/50 flex gap-4 shrink-0">
                                <button
                                    ...
                                    className="flex-1 h-12 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                ...
                                <button
                                    ...
                                    className="px-6 h-12 rounded-2xl bg-zinc-800 border border-red-500/10 text-red-500 hover:bg-red-500/20 font-bold hover:shadow-[0_0_15px_rgba(239,68,68,0.2)] transition-all"
                                >
