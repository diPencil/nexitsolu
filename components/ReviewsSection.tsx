...
export default function ReviewsSection({
    ...
    return (
        <div
            className={`${embedded ? "mt-0 pt-2" : "mt-20 border-t border-border pt-12"} w-full ${lang === 'ar' ? "dir-rtl text-right" : "dir-ltr text-left"}`}
        >
            <h2 className="text-2xl md:text-3xl font-bold mb-8 flex items-center gap-3">
                <Star className="w-8 h-8 text-yellow-500 fill-yellow-500" />
                {lang === 'ar' ? 'تقييمات العملاء' : 'Customer Reviews'}
                <span className="text-lg text-muted-foreground ml-2 font-mono">({reviews.length})</span>
            </h2>
                        ...
                        {session ? (
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <button
                                    ...
                                    className="w-full bg-[#0066FF] hover:bg-white hover:text-[#0066FF] text-foreground font-bold py-3 px-4 rounded-xl flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                            ...
                            <div key={review.id} className="bg-card border border-border rounded-3xl p-6 transition-all hover:bg-background/70 shadow-lg">
                                <div className="flex items-start justify-between mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-foreground text-sm">{review.user.name || review.user.username || 'User'}</h4>
                                        ...
                                        {session?.user && (session.user as any).id === review.userId && review.status === 'APPROVED' && (
                                            <div className="mt-6 flex gap-2">
                                                ...
                                                <button
                                                    ...
                                                    className="bg-[#0066FF] hover:bg-white hover:text-[#0066FF] text-foreground px-6 rounded-2xl text-[10px] font-black transition-all disabled:opacity-50 shadow-lg shadow-blue-500/10 active:scale-95"
                                                >
