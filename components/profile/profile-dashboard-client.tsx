...
export function ProfileDashboardClient({
    ...
    /** When omitted, shows overview (dashboard). */
    ...
    const activeTab = (() => {
        if (!sectionSeg || sectionSeg === "overview") return "dashboard"
        if (PROFILE_SECTIONS.has(sectionSeg)) return sectionSeg
        return "dashboard"
    })()
    ...
    useEffect(() => {
        if (!sectionSeg) return
        if (!PROFILE_SECTIONS.has(sectionSeg)) {
            router.replace(`/profile/${encodeURIComponent(username)}`)
        }
    }, [sectionSeg, username, router])

    useEffect(() => {
        if (status !== "authenticated") return
        if (!isCompany && ["subscriptions", "contracts", "invoices"].includes(activeTab)) {
            router.replace(`/profile/${encodeURIComponent(username)}`)
        }
    }, [status, isCompany, activeTab, username, router])
    ...
    useEffect(() => {
        if (status === "unauthenticated") {
            ...
            if (window.location.pathname === '/profile') {
                const cleanSlug = sessionSlug ? sessionSlug.toLowerCase().replace(/\s+/g, '-') : ''
    ...
    return (
        ...
        >
            <div className="max-w-[1600px] mx-auto w-full min-w-0">
                <div className="hidden md:flex w-full justify-end mb-4" aria-hidden />
                    ...
                    <div className="md:col-span-3 flex flex-col min-h-0 md:min-h-[600px] w-full min-w-0">
                        ...
                        {isCompany && !isProfileComplete && (
                            <motion.div 
                                ...
                                className="mb-4 md:mb-6 p-3.5 sm:p-6 rounded-xl md:rounded-[2.5rem] bg-red-500/5 border border-red-500/20 flex flex-col md:flex-row md:items-center justify-between gap-3 sm:gap-6 cursor-pointer hover:bg-red-500/10 transition-all group overflow-hidden relative shrink-0"
                            >
                                ...
                                {orders.length > 0 && (
                                    <div className="bg-zinc-950 border border-border rounded-xl md:rounded-[2.5rem] p-4 sm:p-8 md:p-10 text-start">
                                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-4 sm:mb-8">
                                            ...
                                            <button onClick={() => goToTab("orders")} className="text-[#0066FF] text-[10px] font-black uppercase tracking-widest hover:underline transition-all">
                        ...
                        {activeTab === "subscriptions" && isCompany && (
                            <div className="bg-zinc-950 border border-[#0066FF]/20 rounded-xl md:rounded-[2.5rem] p-4 sm:p-8 md:p-10 relative overflow-hidden h-full min-w-0">
                                ...
                                <div className="flex justify-between items-center mb-5 md:mb-8">
                                    <h2 className="text-xl md:text-2xl font-bold flex items-center gap-2 md:gap-3">
                                        <Briefcase className="w-5 h-5 md:w-6 md:h-6 text-[#0066FF] shrink-0" />
                                        ...
                                        {orders.map((order: any) => (
                                            ...
                                            >
                                                ...
                                                <div className="flex w-full items-center justify-between gap-4 sm:w-auto sm:justify-end sm:gap-6">
                                                    <div className="text-start sm:text-end sm:hidden">
                                                        <p className="text-base font-black text-[#0066FF] leading-tight">{order.total} {lang === 'ar' ? 'ج.م' : 'EGP'}</p>
                                ...
                                <div className="grid gap-2 sm:grid-cols-2">
                                        ...
                                        <>
                                            ...
                                            >
                                                <span className="flex items-center gap-3">
                                                    ...
                                                    {lang === "ar" ? "بوابة الشركات" : "Corporate Portal"}
                                                </span>
                                                <ArrowLeft
                                            ...
                                            >
                                                <span className="flex items-center gap-3">
                                                    ...
                                                    {lang === "ar" ? "العقود والطلبات" : "Contracts & Requests"}
                                                </span>
                                                <ChevronRight
                                                    className={`h-4 w-4 ${lang === "ar" ? "rotate-180" : ""}`}
                                                />
                                    ...
                                    >
                                        <span className="flex items-center gap-3">
                                            ...
                                            {lang === "ar" ? "قائمة الأماني" : "Wishlist"}
                                        </span>
                                        <ChevronRight
                                            className={`h-4 w-4 ${lang === "ar" ? "rotate-180" : ""}`}
                                        />
                                    ...
                                    {(session?.user as any)?.role === "ADMIN" && (
                                        <Link
                                            href="/admin"
                                            className="flex w-full items-center justify-between rounded-xl border border-[#0066FF]/20 bg-[#0066FF]/10 px-4 py-3 text-sm font-bold text-[#0066FF] transition-colors hover:bg-[#0066FF]/20"
                                        >
                                            <span className="flex items-center gap-3">
